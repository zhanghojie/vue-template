      ...
      <div>
        <div>
          ...
          <div class="jc" v-if="isActive">
            <div>
              <p><span>联系人：刘子翔</span></p>
              <p><span>联系方式：15112562167</span></p>
            </div>
            <div>
              <p><span>联系人：王杰</span></p>
              <p><span>联系方式：15112562167</span></p>
            </div>
          </div>
...
<script>
import {getJcInfoAPI} from 'api/home';
...
export default {
  ...
  mounted() {
    let InfoParams = this.$route.params;
    getJcInfoAPI({zfid: InfoParams.zfid})
      .then(res => {
        ...
        for (let key in result) {
          if (Object.hasOwnProperty.call(result, key)) {
            if (key == 'zfPlanStartTime' && key != null) {
              ...
              zfPlanStartTime = dayjs(zfPlanStartTime).format('YYYY-MM-DD');
            }
            if (key == 'zfPlanEndTime' && key != null) {
              let zfPlanEndTime = result[key];
              zfPlanEndTime = dayjs(zfPlanEndTime).format('YYYY-MM-DD');
            }
          }
        }
        console.log(result);
