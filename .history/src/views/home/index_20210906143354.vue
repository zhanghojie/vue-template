...
export default {
  ...
  methods: {
    ...
    onRefresh() {
      this.list = [];
      (this.finished = false), (this.loading = true);
      this.onLoad();
    },
    onLoad() {
      let data = {
      ...
      getJcListAPI(data)
        .then(res => {
          ...
          if (result) {
            result.forEach(item => {
              if (item.zfyqEndTime != null) {
                item.zfyqStartTime = dayjs(item.zfyqStartTime).format('YYYY-MM-DD');
                item.zfyqEndTime = dayjs(item.zfyqEndTime).format('YYYY-MM-DD');
              }
            });
