              ...
              <template v-if="list.length">
                <div class="main-main" v-for="(i, index) of list" :key="index + 'list'" @click="handerClick(i)">
                  ...
                  <div class="main-main-jd">
                    ...
                    <div>
                      <van-progress :percentage="70" stroke-width="8" color="#1677FF" :show-pivot="false" />
                      <!-- <div>{{ i.progress }}</div> -->
                      <div>70%</div>
                    </div>
                  </div>
  ...
  methods: {
    ...
    initData() {
      ...
      function onDeviceReady() {
        function loadData() {
          HTMICordovaSuport.getConfigValue('host', function (configResult) {
            global.set_host(configResult);

            HTMICordovaSuport.getSystemVariables('|app_id|,|user_id|,|user_name|', function (res) {
              let user_id = res.user_id;
              let app_id = res.app_id;
