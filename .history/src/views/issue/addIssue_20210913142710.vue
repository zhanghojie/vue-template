<template>
  <div class="container">
    ...
    <div class="upload">
      <van-uploader @delete="uploadDelete" :max-count="9" :after-read="afterRead" upload-icon="plus" :upload-text="uploadIndex + '/9'" v-model="fileList" />
    </div>
    <div class="add">
      <van-button class="btn" size="large" @click="saveClick" block>保存</van-button>
    </div>

    <van-action-sheet v-model="show1" :actions="actions1" @select="onSelect1" @opened="onOpened1" cancel-text="取消" close-on-click-action />
    <van-action-sheet v-model="show2" :actions="actions2" @select="onSelect2" @opened="onOpened2" cancel-text="取消" close-on-click-action />
    <van-action-sheet v-model="show3" :actions="actions3" @select="onSelect3" @opened="onOpened3" cancel-text="取消" close-on-click-action />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    saveClick() {
      let inexistence = ['', null, undefined];
      if (!inexistence.includes(this.filedValue)) {
        let data = {
          userId: this.userId,
          zfid: this.zfid,
    ...
    onOpened1() {
      if (!this.actions1.length) {
        this.$toast('当前没有选择类别');
        this.show1 = false;
      }
    },
    onOpened2() {
      if (!this.actions2.length) {
        this.$toast('当前没有选择类别');
        this.show2 = false;
      }
    },
    onOpened3() {
      if (!this.actions3.length) {
        this.$toast('当前没有选择类别');
        this.show3 = false;
      }
    }
  ...
  mounted() {
    getIssuesClassifyAPI({userId: this.userId, zfid: this.zfid})
      .then(res => {
      ...
      .then(() => {
        ...
        if (issueId) {
          ...
          updateIssueInfoAPI({userId: this.userId, issueId})
            .then(res => {
              let result = res?.result;
