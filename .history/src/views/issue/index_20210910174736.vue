                ...
                <template v-if="i.visitImg.length">
                  <div>
                    <template v-for="(item, index) of i.visitImg">
                      <template v-if="item.fk"><img :src="item.fk" alt="" :key="index + 'img'" @click.stop="imageClick(i.visitImg, index)" /></template>
                    </template>
                  </div>
                </template>
                <p>{{ i.issueDesc }}</p>
      ...
      <template v-if="active2">
        <van-list v-model="loading2" :finished="finished2" finished-text="没有更多了" @load="onLoad2">
          <template v-if="list2.length">
            <div class="wrap-bottom" v-for="(i, index) of list2" :key="index + 'certain'">
              <h2>问题{{ i.issueOrder }}</h2>
              <p>监察指标：</p>
              <p>是否合格：</p>
              <template v-if="i.visitImg.length">
...
export default {
  ...
  methods: {
    ...
    handlerdelete(issueId) {
      this.$dialog
        ...
        .then(() => {
          deleteIssueAPI({userId: this.userId, issueId})
            .then(res => {
              this.$toast('删除成功');
              this.onRefresh();
              this.getCount();
    ...
    getCount() {
      getIssuesCountAPI({userId: this.userId, zfid: this.zfid})
        .then(res => {
  ...
  mounted() {
    getJudgeOperationButtonAPI({userId: this.userId, zfid: this.zfid})
      .then(res => {
        let result = res?.result;
